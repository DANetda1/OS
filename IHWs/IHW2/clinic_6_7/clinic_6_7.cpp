...
const char* SEM_QUEUE_MUTEX = "/queue_mutex";
const char* SEM_TREATED_MUTEX = "/treated_mutex";
const char* SEM_PATIENT_READY = "/patient_ready";
const char* SEM_DOCTOR[3] = { "/doctor_0", "/doctor_1", "/doctor_2" };
const char* SEM_PRINT_MUTEX = "/print_mutex"; // [Изменение]
...
sem_t* sem_treated_mutex;
sem_t* sem_patient_ready;
sem_t* sem_doctor[3];
sem_t* sem_print; // [Изменение]
...
void cleanup() {
    ...
    sem_close(sem_treated_mutex);
    sem_close(sem_patient_ready);
    for (int i = 0; i < 3; ++i)
        sem_close(sem_doctor[i]);
    sem_close(sem_print); // [Изменение]

    if (getpid() == parent_pid) {
        sem_unlink(SEM_QUEUE_MUTEX);
        sem_unlink(SEM_TREATED_MUTEX);
        sem_unlink(SEM_PATIENT_READY);
        for (int i = 0; i < 3; ++i)
            sem_unlink(SEM_DOCTOR[i]);
        sem_unlink(SEM_PRINT_MUTEX); // [Изменение]
    }
}
...
void doctor(Specialty spec) {
    ...
    while (true) {
        ...
        sem_post(sem_print); // [Изменение]
        sem_post(sem_doctor[spec]);

        sem_wait(sem_treated_mutex);
        data->treated_patients++;
        sem_post(sem_treated_mutex);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 2) {
        std::cerr << "Использование: ./clinic_6_7 <кол-во_пациентов>\n";
        return 1;
    }
    ...
    sem_queue_mutex = sem_open(SEM_QUEUE_MUTEX, O_CREAT, 0600, 1);
    sem_treated_mutex = sem_open(SEM_TREATED_MUTEX, O_CREAT, 0600, 1);
    sem_patient_ready = sem_open(SEM_PATIENT_READY, O_CREAT, 0600, 0);
    for (int i = 0; i < 3; ++i)
        sem_doctor[i] = sem_open(SEM_DOCTOR[i], O_CREAT, 0600, 1);
    sem_print = sem_open(SEM_PRINT_MUTEX, O_CREAT, 0600, 1); // [Изменение]
